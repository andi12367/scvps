...
clear;clear;clear
echo -e " ${CYAN} ┌─────────────────────────────────────────────────┐${NC}"
echo -e " ${CYAN} │ ${BG_RED}               TEMPEST TUNNELING                  ${NC} ${CYAN}│$NC"
echo -e " ${CYAN} └─────────────────────────────────────────────────┘${NC}"
echo ""
...
clear
rm -f /usr/bin/user
username=$(curl https://raw.githubusercontent.com/andi12367/scvps/main/izin | grep $MYIP | awk '{print $2}')
echo "$username" >/usr/bin/user
expx=$(curl https://raw.githubusercontent.com/andi12367/scvps/main/izin | grep $MYIP | awk '{print $3}')
echo "$expx" >/usr/bin/e
...
Exp1=$(curl https://raw.githubusercontent.com/andi12367/scvps/main/izin | grep $MYIP | awk '{print $4}')
if [[ $today < $Exp1 ]]; then
sts="${Info}"
else
sts="${Error}"
...
echo -e "\e[32mloading...\e[0m"
clear
REPO="https://raw.githubusercontent.com/andi12367/scvps/main/"
start=$(date +%s)
secs_to_human() {
...
clear
restart_system() {
USRSC=$(wget -qO- https://raw.githubusercontent.com/andi12367/scvps/main/izin | grep $ipsaya | awk '{print $2}')
EXPSC=$(wget -qO- https://raw.githubusercontent.com/andi12367/scvps/main/izin | grep $ipsaya | awk '{print $3}')
...
mkdir -p /etc/kyt/file/vmess/ip
mkdir -p /etc/kyt/file/vless/ip
mkdir -p /etc/kyt/file/trojan/ip
mkdir -p /etc/kyt/file/ssh/ip
mkdir -p /etc/file/vmess
mkdir -p /etc/file/vless
mkdir -p /etc/file/trojan
mkdir -p /etc/file/ssh
chmod +x /var/log/xray
touch /etc/xray/domain
touch /var/log/xray/access.log
touch /var/log/xray/error.log
...
wget raw.githubusercontent.com/andi12367/scvps/main/file/limit.sh && chmod +x limit.sh && ./limit.sh
cd
